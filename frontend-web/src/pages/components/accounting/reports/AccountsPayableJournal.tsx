...
export default function AccountsPayableJournal() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const { data } = await napi.get(`/accounts-payable/report/${ticket}/status`);
        setStatus(data);
  ...
  const download = async () => {
    ...
    const url = `/accounts-payable/report/${ticket}/download`;
    ...
    a.href = href;
    a.download = status.format === 'pdf' ? 'accounts_payable_journal.pdf' : 'accounts_payable_journal.xls';
    a.click();
  ...
  const viewPdf = () => {
    ...
    window.open(`/accounts-payable/report/${ticket}/view`, '_blank', 'noopener');
  };
