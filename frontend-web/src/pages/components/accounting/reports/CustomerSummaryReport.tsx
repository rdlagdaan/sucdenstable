...
export default function CustomerSummaryReport() {
  ...
  // load customers (re-uses your existing /customers endpoint like Sales Journal)
  useEffect(() => {
    (async () => {
      try {
        const { data } = await napi.get('/customers', { params: { company_id: companyId } });
        const mapped: DropdownItem[] = (data || []).map((c: any) => ({
          code: String(c.cust_id ?? c.code),
          description: String(c.cust_name ?? c.description ?? ''),
        }));
        setCustomers(mapped);
      } catch {
        setCustomers([]);
  ...
  const selectedCustomerName = useMemo(() => {
    const hit = customers.find(c => String(c.code) === String(custId));
  ...
  const begin = async (requested: 'pdf'|'excel') => {
    ...
    setBusy(true);
    try {
      const { data } = await napi.post('/customer-summary/report', {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const { data } = await napi.get(`/customer-summary/report/${ticket}/status?company_id=${companyId}`);
        setStatus(data);
  ...
  const download = async () => {
    ...
    const res = await napi.get(
      `/customer-summary/report/${ticket}/download?company_id=${companyId}`,
      { responseType: 'blob' }
    );
    const url = URL.createObjectURL(res.data);
    const a = document.createElement('a');
    a.href = url;
    a.download = status.format === 'pdf' ? 'customer_summary_report.pdf' : 'customer_summary_report.xls';
    a.click();
  ...
  const viewPdf = async () => {
    ...
    const res = await napi.get(
      `/customer-summary/report/${ticket}/view?company_id=${companyId}`,
      { responseType: 'blob' }
    );
    const url = URL.createObjectURL(res.data);
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="text-lg font-semibold">CUSTOMER SUMMARY REPORT</div>
        ...
        <div className="md:col-span-2">
          ...
          {selectedCustomerName && (
            <div className="text-xs text-gray-600 mt-1">
              Selected: <b>{selectedCustomerName}</b>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-5">
            ...
            <div className="mb-2 text-sm">
              ...
              {selectedCustomerName && <> <span className="mx-2">•</span> Customer: <b>{selectedCustomerName}</b> </>}
            </div>
