...
class BuildAccountsReceivableJournal implements ShouldQueue
{
    ...
    private function buildPdf(string $file, callable $progress): void
    {
        ...
        $pdf = new class($co, $from, $to) extends \TCPDF {
            ...
            {
                // Fixed layout header (NO big HTML blocks)
                ...
                $this->Cell(0, 7, 'SALES JOURNAL', 0, 1, 'L');
        ...
        // ✅ Single, correct margin setup (DO NOT override later)
        ...
        $this->applyFilters($this->baseQuery())
            ...
            ->chunk(200, function ($chunk) use (&$done, $progress, $pdf) {
                ...
                foreach ($chunk as $row) {
                    ...
                    // Before writing a new transaction header, if we're too low, move to next page
                    if ($pdf->GetY() > 235) {
                        $pdf->AddPage();
                    }
...
HTML;
                    ...
                    foreach ((json_decode($row->lines, true) ?: []) as $ln) {
                        ...
                        // If near bottom, flush current table and page break cleanly
                        if ($pdf->GetY() > 245) {
                            $rowsHtml .= '</table>';
