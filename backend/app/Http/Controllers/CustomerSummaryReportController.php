...
use App\Jobs\BuildCustomerSummaryReport;
...
public function start(Request $request)
{
    try {
        ...
        $cust  = (string)$request->input('cust_id');
        ...
        if (!$cust) return response()->json(['message' => 'Missing cust_id'], 422);
        ...
        // ✅ seed cache using default Cache store (same as working modules)
        ...
        // ✅ dispatch pattern same as working modules
        BuildCustomerSummaryReport::dispatchAfterResponse(
            ticket: $ticket,
            startDate: $start,
            endDate: $end,
            custId: $cust,
        ...
        \Log::error('CustomerSummaryReportController@start failed', [
            'error' => $e->getMessage(),
...
// ✅ Alias endpoint: some routes/UI call /api/customer-summary/report
// If your routes already point /report to start(), this is harmless.
// If your routes accidentally point to report(), this fixes the 500 immediately.
public function report(Request $request)
{
    ...
    public function status(Request $request, string $ticket)
    {
        ...
        // tenant safety: ensure same company
...
public function download(Request $request, string $ticket)
{
    try {
        ...
        $name = $ext === 'pdf'
            ? 'customer_summary_report.pdf'
            : 'customer_summary_report.xls';
        ...
        \Log::error('CustomerSummaryReportController@download failed', [
            'ticket' => $ticket,
            'error'  => $e->getMessage(),
            'trace'  => $e->getTraceAsString(),
        ]);
...
public function view(Request $request, string $ticket)
{
    try {
        ...
        return response()->file($absolute, [
            ...
            'Content-Disposition' => 'inline; filename="customer_summary_report.pdf"',
            'Cache-Control'       => 'private, max-age=0, must-revalidate',
            'Pragma'              => 'public',
        ]);
    } catch (\Throwable $e) {
        \Log::error('CustomerSummaryReportController@view failed', [
            'ticket' => $ticket,
            'error'  => $e->getMessage(),
            'trace'  => $e->getTraceAsString(),
        ]);
