...
class SoImportedRefinedSugarController extends Controller
{
    private string $mainTable = 'so_imported_refined_sugar';
    private string $detailTable = 'so_imported_refined_sugar_details';
    private string $blSelectionTable = 'so_imported_refined_sugar_bls';
    private string $processEntryTable = 'so_imported_refined_sugar_process_entries';

    public function list(Request $request): JsonResponse
    {
        $companyId = (int) $request->query('company_id');

        $rows = DB::table($this->mainTable)
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        $savedBl = DB::table($this->blSelectionTable)
            ->where('company_id', $companyId)
            ->where('so_imported_refined_sugar_id', $id)
            ->where('selected_flag', true)
            ->orderBy('id')
            ->first(['bill_of_lading_id', 'bl_no']);

        $details = DB::table($this->detailTable)
            ->where('so_imported_refined_sugar_id', $id)
            ->where('company_id', $companyId)
            ->where(function ($q) {
                $q->whereNull('delete_flag')->orWhere('delete_flag', false);
        ...
        $main->item_label = $poItem?->particulars ?: 'REFINED SUGAR';
        $main->bl_id = $main->bl_id ?? $savedBl?->bill_of_lading_id ?? null;
        $main->bl_no = $main->bl_no ?? $savedBl?->bl_no ?? '';
    ...
    public function storeMain(Request $request): JsonResponse
    {
        ...
        return DB::transaction(function () use ($data, $request) {
            ...
            if (Schema::hasColumn($this->mainTable, 'vat_amount')) {
                $insert['vat_amount'] = $data['vat_amount'] ?? 0;
            }

            $id = DB::table($this->mainTable)->insertGetId($insert);
    ...
    public function updateMain(Request $request): JsonResponse
    {
        ...
        if (Schema::hasColumn($this->mainTable, 'vat_amount')) {
            $update['vat_amount'] = $data['vat_amount'] ?? 0;
        }
    ...
    {
        $data = $request->validate([
            'so_imported_refined_sugar_id' => ['required', 'integer'],
            'company_id' => ['required', 'integer'],
        ...
        $main = DB::table($this->mainTable)
            ->where('id', (int) $data['so_imported_refined_sugar_id'])
            ->where('company_id', (int) $data['company_id'])
            ->first();
        ...
        $id = DB::table($this->detailTable)->insertGetId([
            'so_imported_refined_sugar_id' => (int) $data['so_imported_refined_sugar_id'],
            'company_id' => (int) $data['company_id'],
        ...
        $this->recalcQuantity((int) $data['so_imported_refined_sugar_id'], (int) $data['company_id']);
    ...
    public function updateDetail(Request $request): JsonResponse
    {
        ...
        DB::table($this->detailTable)
            ->where('id', (int) $data['id'])
            ->where('so_imported_refined_sugar_id', (int) $data['so_imported_refined_sugar_id'])
            ->where('company_id', (int) $data['company_id'])
            ->update([
                'row_no' => (int) $data['row_no'],
                'so_type' => $data['so_type'],
    ...
    public function siDropdown(Request $request): JsonResponse
    {
        ...
        $rows = DB::table($this->mainTable)
            ->where('company_id', $companyId)
            ->where(function ($w) {
                $w->whereNull('delete_flag')->orWhere('delete_flag', false);
            })
        ...
        $rows = $detailQ
            ...
            ->map(function ($r) {
                ...
                return [
                    ...
                    'item_label' => $particulars !== '' ? $particulars : 'REFINED SUGAR',
    ...
    public function blDropdown(Request $request): JsonResponse
    {
        ...
        $rows = DB::table('bill_of_lading as b')
            ->leftJoin('bill_of_lading_line as l', 'l.bill_of_lading_id', '=', 'b.id')
            ->where('b.company_id', $companyId)
            ->where(function ($w) {
                $w->whereNull('b.delete_flag')->orWhere('b.delete_flag', false);
            })
            ->when($poNo !== '', function ($qq) use ($poNo) {
                $qq->where('b.po_no', $poNo);
            })
            ->when($q !== '', function ($qq) use ($q) {
                $like = '%' . strtolower($q) . '%';

                $qq->where(function ($w) use ($like) {
                    $w->whereRaw('LOWER(CAST(COALESCE(b.bl_no, \'\') as text)) LIKE ?', [$like])
            ...
            ->get([
                ...
                'b.vendor_code',
                'b.vendor_name',
                DB::raw('COALESCE(SUM(l.bags), 0) as bags'),
            ]);

        return response()->json($rows);
    }

    public function blLineList(Request $request): JsonResponse
    {
        $companyId = (int) $request->query('company_id');
        $billOfLadingId = (int) ($request->query('bill_of_lading_id') ?: 0);
        $blNo = trim((string) $request->query('bl_no', ''));
        $mainId = (int) ($request->query('so_imported_refined_sugar_id') ?: 0);

        if ($companyId <= 0 || ($billOfLadingId <= 0 && $blNo === '')) {
            return response()->json([]);
        }
        ...
        if ($mainId > 0) {
            $saved = DB::table($this->blSelectionTable)
                ->where('company_id', $companyId)
                ->where('so_imported_refined_sugar_id', $mainId)
                ->get()
                ->keyBy('bill_of_lading_line_id');
        }

        $usedByOthers = DB::table($this->blSelectionTable)
            ->where('company_id', $companyId)
            ->where('selected_flag', true)
            ->when($mainId > 0, function ($q) use ($mainId) {
                $q->where('so_imported_refined_sugar_id', '<>', $mainId);
            })
            ->select('bill_of_lading_line_id', DB::raw('COALESCE(SUM(selected_bags), 0) as used_bags'))
            ->groupBy('bill_of_lading_line_id')
            ->get()
            ->keyBy('bill_of_lading_line_id');

        $rows = DB::table('bill_of_lading_line as l')
            ->join('bill_of_lading as b', 'b.id', '=', 'l.bill_of_lading_id')
            ->where('b.company_id', $companyId)
            ->where(function ($w) {
                $w->whereNull('b.delete_flag')->orWhere('b.delete_flag', false);
            })
            ->when($billOfLadingId > 0, function ($q) use ($billOfLadingId) {
                $q->where('l.bill_of_lading_id', $billOfLadingId);
            })
            ->when($billOfLadingId <= 0 && $blNo !== '', function ($q) use ($blNo) {
                $q->where('l.bl_no', $blNo);
            })
            ->orderBy('l.line_no')
            ->orderBy('l.id')
            ->get([
            ...
            ->map(function ($r) use ($saved, $usedByOthers) {
                $lineId = (int) $r->id;
                $originalBags = (int) ($r->bags ?? 0);

                $savedRow = $saved->get($lineId);
                $otherUsedBags = (int) ($usedByOthers->get($lineId)->used_bags ?? 0);

                $thisSiUsedBags = $savedRow ? (int) $savedRow->selected_bags : 0;
                $remainingBags = max(0, $originalBags - $otherUsedBags);

                return [
                    'id' => $lineId,
                    'bill_of_lading_id' => (int) $r->bill_of_lading_id,
                    ...
                    'purchase_order_line_id' => $r->purchase_order_line_id ? (int) $r->purchase_order_line_id : null,

                    'selected_flag' => $savedRow ? (bool) $savedRow->selected_flag : false,
                    'override_flag' => $savedRow ? (bool) $savedRow->override_flag : false,
                    'override_bags' => $savedRow ? $savedRow->override_bags : null,
                    'selected_bags' => $thisSiUsedBags,

                    'used_by_other_si' => $otherUsedBags,
                    'remaining_bags' => $remainingBags,
                    'available_for_this_si' => $remainingBags + $thisSiUsedBags,
                ];
            })
            ->values();
    ...
    public function saveBlLines(Request $request): JsonResponse
    {
        $data = $request->validate([
            'so_imported_refined_sugar_id' => ['required', 'integer'],
            'company_id' => ['required', 'integer'],
            ...
            'items.*.bill_of_lading_line_id' => ['required', 'integer'],
            'items.*.bill_of_lading_id' => ['required', 'integer'],
            'items.*.bl_no' => ['required', 'string', 'max:100'],
            'items.*.original_bags' => ['required', 'numeric', 'min:0'],
            'items.*.selected_flag' => ['required', 'boolean'],
            'items.*.override_flag' => ['required', 'boolean'],
            'items.*.override_bags' => ['nullable', 'numeric', 'min:0'],
        ]);

        $mainId = (int) $data['so_imported_refined_sugar_id'];
        ...
        if ((bool) ($main->posted_flag ?? false)) {
            return response()->json(['message' => 'Cannot update Bill of Lading selection. Sales Invoice is already posted.'], 409);
        }
        ...
        return DB::transaction(function () use ($data, $mainId, $companyId, $request) {
            ...
            foreach ($data['items'] as $item) {
                $lineId = (int) $item['bill_of_lading_line_id'];
                $originalBags = (int) $item['original_bags'];
                $selectedFlag = (bool) $item['selected_flag'];
                $overrideFlag = (bool) $item['override_flag'];
                $overrideBags = $item['override_bags'] === null ? null : (int) $item['override_bags'];

                $otherUsedBags = (int) DB::table($this->blSelectionTable)
                    ->where('company_id', $companyId)
                    ->where('bill_of_lading_line_id', $lineId)
                    ->where('selected_flag', true)
                    ->where('so_imported_refined_sugar_id', '<>', $mainId)
                    ->sum('selected_bags');

                $availableBags = max(0, $originalBags - $otherUsedBags);

                if ($overrideFlag) {
                    if ($overrideBags === null) {
                        return response()->json([
                            'message' => "Override bags is required for BL line {$lineId}.",
                        ], 422);
                    }

                    if ($overrideBags > $availableBags) {
                        return response()->json([
                            'message' => "Override bags for BL line {$lineId} cannot exceed remaining bags {$availableBags}.",
                        ], 422);
                    }
                }

                $selectedBags = $selectedFlag
                    ? ($overrideFlag ? (int) $overrideBags : $availableBags)
                    : 0;

                if ($selectedFlag && $selectedBags > $availableBags) {
                    return response()->json([
                        'message' => "Selected bags for BL line {$lineId} cannot exceed remaining bags {$availableBags}.",
                    ], 422);
                }

                $incomingLineIds[] = $lineId;
                $totalBags += $selectedBags;

                DB::table($this->blSelectionTable)->updateOrInsert(
                    [
                        'company_id' => $companyId,
                        'so_imported_refined_sugar_id' => $mainId,
                        'bill_of_lading_line_id' => $lineId,
                    ],
                    [
                        'bill_of_lading_id' => (int) $item['bill_of_lading_id'],
                        'bl_no' => $item['bl_no'],
                        'original_bags' => $originalBags,
                        'selected_bags' => $selectedBags,
                        'override_flag' => $overrideFlag,
                        'override_bags' => $overrideFlag ? $overrideBags : null,
            ...
            if (count($incomingLineIds) > 0) {
                DB::table($this->blSelectionTable)
                    ->where('company_id', $companyId)
                    ->where('so_imported_refined_sugar_id', $mainId)
                    ->whereNotIn('bill_of_lading_line_id', $incomingLineIds)
                    ->update([
                        'selected_flag' => false,
                        'selected_bags' => 0,
                        'override_flag' => false,
            ...
            return response()->json([
                'message' => 'Selected Bill of Lading lines saved.',
                'quantity' => $totalBags,
            ]);
        });
    }
    ...
    public function processTransaction(Request $request): JsonResponse
    {
        ...
        $vat = (bool) ($main->vatable_flag ?? false) ? round($gross * 0.12, 2) : 0;
        $arDebit = round($gross + $vat, 2);
        ...
        if ($vat > 0) {
            $lines[] = [
                'line_no' => 3,
                'acct_code' => '2201',
                'acct_desc' => $accounts['2201']->acct_desc,
                'debit' => 0,
                'credit' => $vat,
                'remarks' => 'Output VAT',
            ];
        ...
        return DB::transaction(function () use ($data, $lines) {
            DB::table($this->processEntryTable)
                ->where('so_imported_refined_sugar_id', $data['id'])
                ->where('company_id', $data['company_id'])
                ->delete();

            foreach ($lines as $line) {
                DB::table($this->processEntryTable)->insert([
                    'so_imported_refined_sugar_id' => $data['id'],
    ...
    public function processToSalesJournal(Request $request): JsonResponse
    {
        ...
        $entries = DB::table($this->processEntryTable)
            ->where('so_imported_refined_sugar_id', $sourceId)
            ->where('company_id', $companyId)
            ->orderBy('line_no')
            ->get();
        ...
        return DB::transaction(function () use ($request, $data, $companyId, $main, $entries, $sumDebit, $sumCredit) {
            ...
            $cashSalesId = DB::table('cash_sales')->insertGetId([
                ...
                'amount_in_words' => $this->amountToWords($salesAmount),
                'booking_no' => null,
                'is_cancel' => 'n',
    ...
    public function formPdf(Request $request, $id = null)
    {
        ...
        $isVatable = (bool) ($main->vatable_flag ?? false);

        $vatableSales = $isVatable ? $gross : 0;
        $vatAmount = $isVatable ? round($gross * 0.12, 2) : 0;
        $vatExemptSales = $isVatable ? 0 : $gross;
        $totalDue = round($gross + $vatAmount, 2);
        ...
        $mainDescLineX = $x + $descW;
        $mainQtyLineX = $x + $descW + $qtyW;
        $mainAmountLineX = $x + $descW + $qtyW + $priceW;
        ...
        if ($main) {
            ...
            $pdf->SetXY($x + 2, $lineY);
            $pdf->Cell($descW - 4, 5, 'REFINED SUGAR', 0, 0, 'L');
            ...
            $pdf->Cell($amtW - 2, 5, $gross ? number_format($gross, 2) : '', 0, 0, 'R');
        }
        ...
        $rightH = 44;
        $rightSeparatorX = $mainAmountLineX;
        ...
        $labels = [
            'Total Sales (VAT Inclusive)' => $totalDue,
            'Less : VAT' => $vatAmount,
            'Amount : Net of VAT' => $gross,
            'Add : VAT' => $vatAmount,
        ...
        return response($bytes, 200)
            ...
            ->header('Content-Disposition', 'inline; filename="sales-invoice-imported-refined-sugar.pdf"')
